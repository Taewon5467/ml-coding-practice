...
# head( ) 함수를 출력하여 타이터닉 데이터셋의 구성을 간단히 살표보기
...
plt.xlabel('Age')
plt.ylabel('Fare')
plt.legend(handles=scatter.legend_elements()[0], title='Survuved',
...
"""* 에러바는 데이터의 표준 편차를 나타내는 경우에는 길이가 길수록 해당 그룹의 데아터가 퍼져 있음을 의미"""
...
# plt.subplots() 함수를 이용하여 하나의 그림인 figure에 개별 서브플롯을 동시에 추가
...
# 두 번째 서브플롯 설정 (막대 그래프)
axes[1].bar(x, y2, '-o', color='indigo', makersize=7, linewidth=3, alpha=0.7, label='Not Survived')